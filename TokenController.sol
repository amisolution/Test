...
///  affecting the original token
/// @dev It is ERC20 compliant, but still needs to under go further testing.
/// testing new ways to use the contract https://github.com/amisolution/Test/tk.sol
...
contract MiniMeToken is Controlled {
    ...
    string public name = 'AMIS';			//The Token's name: e.g. AMIS Tokens
    uint8 public decimals = '3';			//Number of decimals of the smallest unit
    string public symbol = 'AMIS';			//An identifier: e.g. AMIS
    string public version = 'AMIS_0.1'; 	//A versioning scheme
...
///////////////////
    ...
    function doTransfer(address _from, address _to, uint _amount
           ...
           // Alerts the token controller of the transfer
           if (isContract(controller)) {
               if (!TokenMediator(controller).onTransfer(_from, _to, _amount))
               throw;
           }
    ...
    function approve(address _spender, uint256 _amount) returns (bool success) {
        ...
        // Alerts the token controller of the approve function call
        if (isContract(controller)) {
            if (!TokenMediator(controller).onApprove(msg.sender, _spender, _amount))
                throw;
        }
...
////////////////
    ...
    function ()  payable {
        if (isContract(controller)) {
            if (! TokenMediator(controller).proxyPayment.value(msg.value)(msg.sender))
                throw;
        } else {
            throw;
        }
